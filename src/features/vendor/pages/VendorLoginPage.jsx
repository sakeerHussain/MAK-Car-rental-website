...
export default function VendorLoginPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    ...
    defaultValues: { email: 'vendor@mak.ae', password: 'vendor123' },
  });
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (user.role !== 'VENDOR') {
        setError('This account does not have vendor portal access.');
        return;
      }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-primary-deep px-4">
      <Card className="w-full max-w-md">
        <CardHeader>
          <CardTitle>MAK Vendor Portal</CardTitle>
          <CardDescription>Sign in with vendor@mak.ae (demo mode)</CardDescription>
        </CardHeader>
        <CardContent>
